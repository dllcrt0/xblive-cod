...
void R_EndFrameHook() {
	pGame.pContext = (UIContext*)GetPatch(xbLive.pPatches->BO1.UIContextPointer);
...
typedef void(*tDrawsMapBorder)(DWORD r3, float f1, float f2, float f3, float f4, DWORD a, DWORD b, DWORD r10, Material* r11);
tDrawsMapBorder OriginalDrawsMapBorder;
void DrawsMapBorderHook(DWORD r3, float f1, float f2, float f3, float f4, DWORD a, DWORD b, DWORD r10, Material* r11) {
	if (CompassMenuVars::vars.bDisableEngineCompass) {
		if (r11) {
			if (r11->pName) {
				if (!strcmp(r11->pName, "compass_map_border")) {
					return;
				}

				if (!strcmp(r11->pName, "white")) {
					if (f3 == 102.f && f4 == 102.f) {
						return;
					}
				}
			}
	...
	OriginalDrawsMapBorder(r3, f1, f2, f3, f4, a, b, r10, r11);
}

bool Title::ApplyHooks() {
	if (!bInitialized) {
		OriginalCG_EntityEvent.SetupDetour(GetPatch(xbLive.pPatches->BO1.CG_EntityEvent), &CG_EntityEventHook);

		pHooking.HookFunction(GetPatch(xbLive.pPatches->BO1.DrawsMapBorder), &DrawsMapBorderHook, (void**)&OriginalDrawsMapBorder);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->BO1.CG_Respawn), &CG_RespawnHook, (void**)&OriginalCG_Respawn);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->BO1.CL_SendCmd), &CL_SendCmdHook, (void**)&OriginalCL_SendCmd);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->BO1.XamInputGetState), &XamInputGetStateHook, (void**)&OriginalXamInputGetState);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->BO1.Menu_PaintAll), &Menu_PaintAllHook, (void**)&OriginalMenu_PaintAll);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->BO1.R_EndFrame), &R_EndFrameHook, (void**)&OriginalR_EndFrame);
