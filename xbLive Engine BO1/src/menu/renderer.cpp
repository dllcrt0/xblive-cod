...
void Renderer::Initialize() {
	...
	Materials.map_grid_square = Engine::R_RegisterMaterial("map_grid_square", 0);
	Materials.compass_map_default = Engine::R_RegisterMaterial("compass_map_default", 0);
	Materials.tow_filter_overlay_no_signal = Engine::R_RegisterMaterial("tow_filter_overlay_no_signal", 0);
	...
	Materials.hud_scavenger_pickup = Engine::R_RegisterMaterial("hud_scavenger_pickup", 0);
	...
	Fonts.normalFont = Engine::R_RegisterFont("fonts/smallfont", 0);
...
void Renderer::DrawTopRight(const char* text, int pos) {
	...
	DrawString(text, CalculateXValueWithAlign(1265.f, text, Fonts.normalFont, 0.54f, JustifyLeft), 20.f + (pos * 29.f) - 1.f, Fonts.normalFont, 0.54f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->BO1.ScreenPlacement), X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}
