...
namespace ESPMenuVars {
	...
	void PatchGameNames() {
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.PatchGameNames) = (vars.bDrawNames || vars.bDrawDistance) ? 0x60000000 : 0x4BFFEF21;
	}
	...
	if (Engine::InGame()) {
					...
					if (vars.bDrawBox) {
						...
						if (vars.iDrawBoxSelection == 3) {
							...
							if (Engine::WorldToScreen(position.vOrigin, &origin) && Engine::WorldToScreen(position.vHead, &head)) {
								...
								float width = Engine::GetEntity(i).GetFlags() & 8 ? height / 0.9f : height / 1.95f;
		...
		if (vars.bDrawDogs ) {
			for (int i = 18; i < 0x7FF; i++) {
				auto entity = Engine::GetEntityPtr();
				if (Engine::IsAlive(i)) {
					if (entity[i].GetType() == ET_ACTOR_SPAWNER) {
						bool isFriend = Engine::GetCG()->ClientInfo[Engine::GetCG()->iClientNumber].iTeam == entity[i].GetTeam();
