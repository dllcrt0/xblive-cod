...
namespace MainMenuVars {
	...
	void NoRecoil() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO1.NoRecoil), vars.bNoRecoil ? 0x60000000 : 0x4BF67EC5); 
	}

	void NoSway() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO1.NoSway), vars.bNoSway ? 0x60000000 : 0x4BFFEB09);
	}

	void NoFlinch() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO1.NoFlinch), vars.bNoFlinch ? 0x60000000 : 0x4BFFF77D);
	}

	void ConstantRadar() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO1.ConstantRadar), vars.bConstantRadar ? 0x60000000 : 0x419A0050);
	}
	...
	void NoSpread(UserCommand* pNextUserCommand, UserCommand* pUserCommand) {
		if (*(float*)((DWORD)Engine::GetCG() + 0x69264) == 0.f) return;
