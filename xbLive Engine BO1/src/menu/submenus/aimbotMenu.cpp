...
namespace AimbotMenuVars {
	...
	bool IsClientWallbangable(int client, const char* bone) {
		...
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[0]) = 0x4E800020;
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[1]) = 0x4E800020;
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[2]) = 0x60000000;
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[3]) = 0x60000000;

		BulletTraceResults bulletTraceResults;
		Engine::FireBulletPenetrate(0, &bulletFireParams, Engine::GetEntity(Engine::GetCG()->iClientNumber).dwWeaponIndex, &Engine::GetEntity(Engine::GetCG()->iClientNumber), &viewOrigin, 0, 1, &bulletTraceResults);

		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[0]) = 0x7D8802A6;
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[1]) = 0x7D8802A6;
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[2]) = 0x409A0008;
		*(DWORD*)GetPatch(xbLive.pPatches->BO1.Autowall[3]) = 0x4BFE2CD9;
	...
	int CalculateBestClient() {
		if (Engine::InGame() && AimbotMenuVars::vars.bAimbot) {
			...
			for (int i = 0; i < 18; i++) {
				...
				bool shooting = entity.bState2 == 0x40;
				bool aiming = entity.bState1 == 0x8;
