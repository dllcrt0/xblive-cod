	...
	if (Engine::InGame()) {
		...
		for (int i = 18; i < 0x7FF; i++) {
				...
				if (Engine::IsAlive(i) && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
					if (Engine::IsWeaponEquipmentItem(entity[i].bWeaponNum)) {
						...
						if (Engine::WorldToScreen(entity[i].vOrigin, &topLocationOut)) {
							...
							if (icon) {
								Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->BO2.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X, aspect.Y, 0.0, vars.DrawEquipmentColor.GetFloatPtr(), icon);
							}
						}
					}
				}
			...
			if (vars.bDrawDroppedItems) {
				bool scav = false;
				if (Engine::IsAlive(i) && entity[i].Type == ET_ITEM && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
					Vector3<float> topLocationOut;
					if (Engine::WorldToScreen(entity[i].vOrigin, &topLocationOut)) {
						...
						if (!scav) {
							...
							auto icon = Engine::GetWeaponMaterialWithAspect(entity[i].bWeaponNum, &aspect);
							if (icon) {
								Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->BO2.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X, aspect.Y, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), icon);
							}
						} else {
							Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->BO2.ScreenPlacement), topLocationOut.X - 30, topLocationOut.Y - 10, 30, 30, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), pRenderer.Materials.specialty_scavenger);
						}
					}
				}
			}
