...
namespace MainMenuVars {
	...
	void NoRecoil() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.NoRecoil), vars.bNoRecoil ? 0x60000000 : 0x48461341);
	}

	void NoSpread(UserCommand* pNextUserCommand, UserCommand* pUserCommand) {
		if (*(float*)((DWORD)Engine::GetCG() + 0x80B80) == 0.f) return;
	...
	void NoSway() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.NoSway[0]), vars.bNoSway ? 0x60000000 : 0x4BFFE975);
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.NoSway[1]), vars.bNoSway ? 0x60000000 : 0x4BFFFA85);
	}

	void NoFlinch() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.NoFlinch[0]), vars.bNoFlinch ? 0x60000000 : 0x4BFFF95D);
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.NoFlinch[1]), vars.bNoFlinch ? 0x60000000 : 0x4BFFAC41);
	}

	void ConstantRadar() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.ConstantRadar[0]), vars.bConstantRadar ? 0x39600001 : 0x817E0000);
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.ConstantRadar[1]), vars.bConstantRadar ? 0x60000000 : 0x40990048);
	}
	...
	void Laser() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->BO2.Laser), vars.bLaser ? 0x38600001 : 0x38600000);
	}
