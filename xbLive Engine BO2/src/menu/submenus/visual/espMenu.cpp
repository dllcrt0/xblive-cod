...
namespace ESPMenuVars {
	...
	void DrawESPLineEasy(Vector3<float> x, Vector3<float> y, Color color) {
		DrawESPLine(Vector3<float>(0.f, 0.f, 0.f), x.X, x.Y, x.Z, y.X, y.Y, y.Z, color);
	}
	...
	void PatchGameNames() {
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.PatchGameNames) = (vars.bDrawNames || vars.bDrawDistance) ? 0x60000000 : 0x4BFFEC35;
	}
...
void ESPMenu::Init() {
	...
	addOption(ToggleOption("Draw Dogs")
		.addToggle(vars.bDrawDogs).canBeSaved("ESP")
		.addTooltip("Draws bones around dogs."));

	addOption(ToggleOption("Draw RCXD")
		.addToggle(vars.bDrawRCXD).canBeSaved("ESP")
		.addTooltip("Draws bones around RCXDs."));
	...
	vars.DrawDogsFriendliesColor = Color(0x1ddbc2FF);
	vars.DrawDogsEnemiesColor = Color(0x1ddbc2FF);

	vars.DrawRCXDFriendliesColor = Color(0x1ddbc2FF);
	vars.DrawRCXDEnemiesColor = Color(0x1ddbc2FF);
}
...
/*Background update*/
void ESPMenu::FeatureUpdatePaintAll() {
	if (Engine::InGame()) {
					...
					if (vars.bDrawBox) {
						...
						if (vars.iDrawBoxSelection == 3) {
							...
							if (Engine::WorldToScreen(position.vOrigin, &origin) && Engine::WorldToScreen(position.vHead, &head)) {
								...
								float width = Engine::GetEntity(i).iFlags & 8 ? height / 0.9f : height / 1.95f;
		...
		if (vars.bDrawDogs || vars.bDrawRCXD) {
			for (int i = 18; i < 0x7FF; i++) {
				auto entity = Engine::GetEntityPtr();
				if (Engine::IsAlive(i)) {
					if (vars.bDrawDogs) {
						if (entity[i].Type == ET_ACTOR) {
							bool isFriend = Engine::GetCG()->ClientInfo[Engine::GetCG()->iClientNumber].iTeam == entity[i].iTeamDog;
							if ((vars.bDrawFriendlies && isFriend) || (vars.bDrawEnemies && !isFriend)) {
								Color color = Color(0xFFFFFFFF);
								if (vars.bDrawFriendlies && isFriend)
									color = vars.DrawDogsFriendliesColor;
								else if (vars.bDrawEnemies && !isFriend)
									color = vars.DrawDogsEnemiesColor;

								ESPMenuVars::DrawBoneOnPlayer(i, "j_knee_le", "j_pelvis", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_elbow_ri", "j_wrist_ri", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_neck", "j_elbow_ri", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_ankle_ri", "j_knee_ri", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_elbow_le", "j_wrist_le", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_ankle_le", "j_knee_le", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_knee_ri", "j_pelvis", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_pelvis", "j_neck", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_neck", "j_elbow_le", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_neck", "j_head", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_head", "j_nose", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_pelvis", "j_tail_base", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_tail_base", "j_tail0", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_tail0", "j_tail1", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_tail1", "j_tail2", color);
								ESPMenuVars::DrawBoneOnPlayer(i, "j_tail2", "j_tail3", color);

								auto headPos = Engine::GetTagPosition(i, "tag_origin");
								auto feetPos = entity[i].vOrigin;
								...
								if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
									...
									FixTextForDistance("Dog", &headPosOut.X, &headPosOut.Y, pRenderer.Fonts.smallFont, 1, feetPos, &distanceOut);
									auto x = pRenderer.CalculateXValueWithAlign(feetPosBackup.X, "Dog", pRenderer.Fonts.smallFont, 0.5f, JustifyCenter);

									pRenderer.DrawRect(x - 2, headPosOut.Y - 27 - 10, (Engine::R_TextWidth("Dog", pRenderer.Fonts.smallFont) * 0.5f) + 2, 10, JustifyLeft, Color(0, 0, 0, 150));
									pRenderer.DrawString("Dog", x - 1, headPosOut.Y - 16 - 10, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);

									char buffer[10];
									_snprintf(buffer, 10, "%.2fm", Engine::GetDistance(Engine::GetEntity(i).vOrigin, Engine::GetEntity(Engine::GetCG()->iClientNumber).vOrigin) * 0.03048f);

									FixTextForDistance(buffer, &headPosOut.X, &headPosOut.Y, pRenderer.Fonts.smallFont, 1, feetPos, &distanceOut);
									x = pRenderer.CalculateXValueWithAlign(feetPosBackup.X, buffer, pRenderer.Fonts.smallFont, 0.5f, JustifyCenter);

									pRenderer.DrawRect(x - 1, headPosOut.Y - 16 - 10, (Engine::R_TextWidth(buffer, pRenderer.Fonts.smallFont) * 0.5f) + 2, 10, JustifyLeft, Color(0, 0, 0, 150));
									pRenderer.DrawString(buffer, x, headPosOut.Y - 5 - 10, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);
								}
							}
						}
					}
					...
					if (vars.bDrawRCXD) {
						if (entity[i].Type == ET_VEHICLE) {
							auto tag = Engine::GetTagPosition(i, "tag_wheel_back_left_steer");
							if (!tag.IsNull()) {
								bool isFriend = Engine::GetCG()->ClientInfo[Engine::GetCG()->iClientNumber].iTeam == entity[i].iTeamRCXD_Dog_Heli;
								if ((vars.bDrawFriendlies && isFriend) || (vars.bDrawEnemies && !isFriend)) {
									Color color = Color(0xFFFFFFFF);
									if (vars.bDrawFriendlies && isFriend)
										color = vars.DrawRCXDFriendliesColor;
									else if (vars.bDrawEnemies && !isFriend)
										color = vars.DrawRCXDEnemiesColor;

									ESPMenuVars::DrawBoneOnPlayer(i, "left_wheel_01_jnt", "tag_body", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "left_wheel_02_jnt", "tag_body", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "right_wheel_01_jnt", "tag_body", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "right_wheel_02_jnt", "tag_body", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "tag_wheel_front_left", "left_wheel_01_jnt", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "tag_wheel_back_left", "left_wheel_02_jnt", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "tag_wheel_front_right", "right_wheel_01_jnt", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "tag_wheel_back_right", "right_wheel_02_jnt", color);
									ESPMenuVars::DrawBoneOnPlayer(i, "tag_body", "tag_player", color);
									...
									if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
										...
										FixTextForDistance("RCXD", &headPosOut.X, &headPosOut.Y, pRenderer.Fonts.smallFont, 1, feetPos, &distanceOut);
										auto x = pRenderer.CalculateXValueWithAlign(feetPosBackup.X, "RCXD", pRenderer.Fonts.smallFont, 0.5f, JustifyCenter);

										pRenderer.DrawRect(x - 2, headPosOut.Y - 27 - 10, (Engine::R_TextWidth("RCXD", pRenderer.Fonts.smallFont) * 0.5f) + 2, 10, JustifyLeft, Color(0, 0, 0, 150));
										pRenderer.DrawString("RCXD", x - 1, headPosOut.Y - 16 - 10, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);
