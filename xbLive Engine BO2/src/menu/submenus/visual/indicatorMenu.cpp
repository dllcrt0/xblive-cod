...
namespace IndicatorMenuVars {
	...
	void DrawPointer(Entity entity, Color pointer, Vector2<float> newScale = Vector2<float>(25.f, 16.f)) {
		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[0]), 75.f); // radius

		auto center = Engine::GetCenter();
		auto offset = entity.vOrigin - Engine::GetCG()->vOrigin;

		float beforeW = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[1]));
		float beforeH = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[2]));

		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[1]), newScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[2]), newScale.Y);

		Engine::CG_DrawGrenadePointer(0, center.X, center.Y, &offset, pointer.GetFloatPtr(), 0.f, 1.0f);

		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[1]), beforeW);
		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadePointer[2]), beforeH);
	}
	...
	void Draw(Entity entity, Material* material, Color pointer, Color icon, bool drawPointer = true, Vector2<float> newIconScale = Vector2<float>(30.f, 30.f)) {
		...
		origScale.X = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->BO2.DrawGrenadeIcon[0]));
		origScale.Y = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->BO2.DrawGrenadeIcon[1]));

		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadeIcon[0]), newIconScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadeIcon[1]), newIconScale.Y);

		Engine::CG_DrawGrenadeIcon(0, center.X, center.Y, &offset, icon.GetFloatPtr(), material, 0.f, 1.0f);

		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadeIcon[0]), origScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->BO2.DrawGrenadeIcon[1]), origScale.Y);
	}
}
	...
	if (Engine::InGame()) {
		...
		if (vars.bDroppedItems || vars.bEquipment) {
			for (int i = 18; i < 0x7FF; i++) {
				...
				if (vars.bEquipment) {
					if (Engine::IsWeaponEquipmentItem(entity[i].bWeaponNum)
						&& !Engine::IsWeaponThis(entity[i].bWeaponNum, WEAPON_SCAV)) {
						if (Engine::IsAlive(i, false)) {
							if ((Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 800.0f)) {
								DrawPointer(entity[i], vars.DrawEquipmentColor);
								Draw(entity[i], Engine::GetWeaponDef(entity[i].bWeaponNum)->pKillIcon, Color(0xFFFFFFFF), Color(0xFFFFFFFF), false, Vector2<float>(18.f, 18.f));
							}
						}
					}
				}
				...
				if (vars.bDroppedItems) {
					if (Engine::IsAlive(i) && entity[i].Type == ET_ITEM && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 800.0f)) {
						if (!Engine::IsWeaponThis(entity[i].bWeaponNum, WEAPON_SCAV)) {
							Vector2<float> aspect;
							auto icon = Engine::GetWeaponMaterialWithAspect(entity[i].bWeaponNum, &aspect);
							if (icon) {
								DrawPointer(entity[i], vars.DrawEquipmentColor);
