...
namespace AimbotMenuVars {
	...
	ScrollStruct<int> ScrollAimTag[] = {
		...
		"j_elbow_ri", 90,
		"j_jaw", 100,
		"j_shouldertwist_le", 80,
		"j_shouldertwist_ri", 80,
		"j_thumb_le_1", 80,
		"j_thumb_ri_1", 80,
		"j_pinky_le_1", 80,
		"j_pinky_ri_1", 80
	...
	bool IsClientWallbangable(int client, const char* bone) {
		...
		if (Engine::IsFriend(client)) return false;
		...
		*(QWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[0]) = 0x386000004E800020;
		*(QWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[1]) = 0x386000004E800020;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[2]) = 0x48000218;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[3]) = 0x38600000;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[4]) = 0x38600000;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[5]) = 0x38600000;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[6]) = 0x38600001;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[7]) = 0x60000000;

		BulletTraceResults bulletTraceResults;
		Engine::FireBulletPenetrate(0, &bulletFireParams, Engine::GetEntity(Engine::GetCG()->iClientNumber).iWeaponIndex, &Engine::GetEntity(Engine::GetCG()->iClientNumber), &viewOrigin, 0, 1, &bulletTraceResults);

		*(QWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[0]) = 0x7D8802A69181FFF8;
		*(QWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[1]) = 0x7D8802A6486BDA55;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[2]) = 0x419A00AC;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[3]) = 0x4BFA1B15;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[4]) = 0x4BFA184D;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[5]) = 0x4BFA17ED;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[6]) = 0x4BFF4301;
		*(DWORD*)GetPatch(xbLive.pPatches->BO2.Autowall[7]) = 0x4BFFF669;
	...
	float GetAimDegree(int t) {
		Vector3<float> clientAngles = *(Vector3<float>*)((DWORD)Engine::GetCG() + 0x69F54 + (t * 0x808));
	...
	int CalculateBestClient() {
		if (Engine::InGame() && AimbotMenuVars::vars.bAimbot) {
			...
			for (int i = 0; i < 18; i++) {
				...
				bool secondaryShield = Engine::IsWeaponThis(entity.iWeaponIndexSecondary, WEAPON_SHIELD);
				...
				if (AimbotMenuVars::vars.bAutowall) {
					...
					if (primaryShield) {
						...
						if (aimDegrees < 70.f) {
							...
							if ((entity.iFlags & 0x4) != 0) {
								auto tag = (rand() % 10) > 4 ? "j_ankle_ri" : "j_knee_ri";
								if ((pGame.Clients[i].bWallbangable = AimbotMenuVars::IsClientWallbangable(i, tag))) {
									pGame.Clients[i].pBestTag = tag;
									pGame.Clients[i].dwPriority += PRIORITY_WALLBANGABLE;
								}
					...
					if (runNormal) {
						...
						if ((pGame.Clients[i].bVisible = IsTargetVisible(Engine::GetBoneFraction(i, pGame.Clients[i].pBestTag)))) {
							pGame.Clients[i].dwPriority += PRIORITY_VISIBLE;
						}
					}
				} else {
