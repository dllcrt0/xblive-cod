...
namespace IndicatorMenuVars {
	...
	void DrawPointer(Entity entity, Color pointer, Vector2<float> newScale = Vector2<float>(25.f, 16.f)) {
		...
		float beforeW = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->MW3.DrawGrenadePointer[0]));
		float beforeH = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->MW3.DrawGrenadePointer[1]));

		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadePointer[0]), newScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadePointer[1]), newScale.Y);

		Engine::CG_DrawGrenadePointer(0, center.X, center.Y, &offset, pointer.GetFloatPtr());

		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadePointer[0]), beforeW);
		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadePointer[1]), beforeH);
	}
	...
	void Draw(Entity entity, Material* material, Color pointer, Color icon, bool drawPointer = true, Vector2<float> newIconScale = Vector2<float>(30.f, 30.f)) {
		...
		origScale.X = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->MW3.DrawGrenadeIcon[0]));
		origScale.Y = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->MW3.DrawGrenadeIcon[1]));

		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadeIcon[0]), newIconScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadeIcon[1]), newIconScale.Y);

		Engine::CG_DrawGrenadeIcon(0, center.X, center.Y, &offset, icon.GetFloatPtr(), material);

		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadeIcon[0]), origScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->MW3.DrawGrenadeIcon[1]), origScale.Y);
	}
}
		...
		if (vars.bDroppedItems || vars.bEquipment) {
			for (int i = 18; i < 0x7FF; i++) {
				...
				if (vars.bDroppedItems) {
					if ((*(int*)((DWORD)&entity[i] + 0x1C4) & 0x1) == 1 && entity[i].Type == ET_ITEM && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
