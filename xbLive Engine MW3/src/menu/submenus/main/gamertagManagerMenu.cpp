...
namespace GamertagManagerMenuVars {
	...
	void SetGamertag(const char* pName) {
		...
		sprintf(szPayload, "userinfo \"\\rate\\20000\\snaps\\20\\name\\%s\\xuid\\%s\\natType\\1\rank\\1\\prestige\\0\"", pName, Engine::GetXenonUserData(0).szXuidString);
		Engine::Cbuf_AddText(0, szPayload);
		strcpy((char*)GetPatch(xbLive.pPatches->MW3.LocalNamePointer), pName);
	}

	void SetGamertagColor(const char* pName, const char* Color) {
		char szPayload[0x100];
		sprintf(szPayload, "userinfo \"\\rate\\20000\\snaps\\20\\name\\%s\\clanAbbrev\\%s\\xuid\\%s\\natType\\1\rank\\1\\prestige\\0\"", pName, Color, Engine::GetXenonUserData(0).szXuidString);
		Engine::Cbuf_AddText(0, szPayload);
		strcpy((char*)GetPatch(xbLive.pPatches->MW3.LocalNamePointer), pName);
	}
	...
	void SetColor() {
		SetGamertagColor(Engine::GetXenonUserData(0).szName, ScrollNameColors[vars.iColor].Result);
	}
}
...
void GamertagManagerMenu::UpdateOnce() {
	...
	call_once(&init, [] {
		strcpy(vars.szDefault, Engine::GetXenonUserData(0).szName);
	});
...
void GamertagManagerMenu::FeatureUpdate() {
	if (vars.bRainbowGamertag) {
		...
		if (GetTickCount() - timer > vars.iRainbowDelay) {
			...
			SetGamertagColor(Engine::GetXenonUserData(0).szName, ScrollNameColors[current].Result);
