				...
				if (Engine::IsAlive(i, true) && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
					if(Engine::IsWeaponEquipmentItem(entity[i].iWeaponIndex)) {
						...
						if (Engine::WorldToScreen(topLocation, &topLocationOut)) {
							...
							if (icon) {
								Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->MW3.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X, aspect.Y, 0.0, vars.DrawEquipmentColor.GetFloatPtr(), icon);
							}
						}
					}
				}
				...
				if (Engine::IsAlive(i, true) && entity[i].Type == ET_ITEM && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
					...
					if (Engine::WorldToScreen(topLocation, &topLocationOut)) {
						...
						if (!scav) {
							...
							auto icon = Engine::GetWeaponMaterialWithAspect(entity[i].iWeaponIndex, &aspect);
							if (icon) {
								Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->MW3.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X * 2.f, aspect.Y, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), icon);
							}
						} else {
							Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->MW3.ScreenPlacement), topLocationOut.X - 30, topLocationOut.Y - 10, 30, 30, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), pRenderer.Materials.specialty_scavenger);
						}
					}
				}
			}
