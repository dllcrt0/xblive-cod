...
void Renderer::DrawTopRight(const char* text, int pos) {
	...
	DrawString(text, 1265.f, 20.f + (pos * 29.f), Fonts.normalFont, 0.54f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->MW3.ScreenPlacement), X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}
...
void Renderer::RenderFooter() {
	...
	if (strTooltip.length() == 0) {
		...
		string tip = WordWrap(strTooltip, Fonts.normalFont, 0.50f, width, &iTooltipLines);
