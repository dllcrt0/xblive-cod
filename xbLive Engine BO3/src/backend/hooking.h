...
public:
	...
	bool HookModuleImport(const char* pModuleName, const char* pImportedModuleName, DWORD dwOrdinal, void* pHookFunction);
	bool HookModuleImport(PLDR_DATA_TABLE_ENTRY module, const char* pImportedModuleName, DWORD dwOrdinal, void* pHookFunction);

	void Unhook(DWORD dwAddress);
	void UnhookAll();

	template <typename T>
	bool HookFunction(DWORD dwAddress, void* pHookFunction, T* pTrampoline) {
		if (dwAddress) {
			HookContext cxt;
			...
			*pTrampoline = (T)startStub;
			...
			LOG_DEV("Hooked: Address=0x%X Stub=0x%X", dwAddress, *pTrampoline);
			return true;
		}
	...
	bool HookFunctionNoTrampoline(DWORD dwAddress, void* pHookFunction) {
		if (dwAddress) {
			HookContext cxt;
			cxt.dwAddress = dwAddress;
			memcpy(cxt.szAsm, (void*)dwAddress, 0x10);
			...
			LOG_DEV("Hooked: Address=0x%X", dwAddress);
			return true;
		}
	...
	virtual DWORD DetourFunctionStart(DWORD dwFunctionAddress, DWORD dwStubAddress, PVOID pDestFunc) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (b == 0x48 || b == 0x4B) {
				...
				pHooking.PatchInJump((PDWORD)(dwStubAddress + dwLength), dwTemp, bTemp);
				dwLength += 0x10;
			...
			// beq or bne, ble or bgt, bge or blt
				...
				branch_else:
					...
					*(DWORD *)(dwStubAddress + dwLength) = ((0x40000000 + (*(DWORD *)dwTempFuncAddr & 0x00FF0000) + 0x14) +
						bTemp ? 0 : 0x01000000);
					dwLength += 4;
					pHooking.PatchInJump((PDWORD)(dwStubAddress + dwLength), (dwTempFuncAddr + dwTemp), FALSE);
					dwLength += 0x10;
				}
			}
		...
		pHooking.PatchInJump((PDWORD)(dwStubAddress + dwLength), (dwFunctionAddress + 0x10), FALSE);
		dwLength += 0x10;

	DoHook:
		// apply the hook
		pHooking.PatchInJump((PDWORD)dwFunctionAddress, (DWORD)pDestFunc, FALSE);
		return dwLength;
	}
