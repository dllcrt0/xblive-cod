							...
							if (Engine::IsWeaponEquipmentItem(entity[i].iWeaponIndex)) {
								if ((Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
									...
									if (Engine::WorldToScreen(entity[i].vOrigin, &topLocationOut)) {
										...
										if (icon) {
											Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->WAW.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X, aspect.Y, 0.0, vars.DrawEquipmentColor.GetFloatPtr(), icon);
										}
									}
								}
							}
							...
							if (index && index < 0x800) {
								if ((Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
									...
									if (Engine::WorldToScreen(entity[i].vOrigin, &topLocationOut)) {
										...
										if (icon) {
											Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->WAW.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X, aspect.Y, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), icon);
										}
									}
								}
							}
