...
namespace IndicatorMenuVars {
	...
	void DrawPointer(Entity entity, Color pointer, Vector2<float> newScale = Vector2<float>(25.f, 16.f)) {
		...
		float beforeW = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->WAW.DrawGrenadePointer[0]));
		float beforeH = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->WAW.DrawGrenadePointer[1]));

		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadePointer[0]), newScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadePointer[1]), newScale.Y);

		Engine::CG_DrawGrenadePointer(0, center.X, center.Y, &offset, pointer.GetFloatPtr());

		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadePointer[0]), beforeW);
		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadePointer[1]), beforeH);
	}
	...
	void Draw(Entity entity, Material* material, Color pointer, Color icon, bool drawPointer = true, Vector2<float> newIconScale = Vector2<float>(30.f, 30.f)) {
		...
		origScale.X = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->WAW.DrawGrenadeIcon[0]));
		origScale.Y = Engine::GetDvar<float>(GetPatch(xbLive.pPatches->WAW.DrawGrenadeIcon[1]));

		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadeIcon[0]), newIconScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadeIcon[1]), newIconScale.Y);

		Engine::CG_DrawGrenadeIcon(0, center.X, center.Y, &offset, icon.GetFloatPtr(), material);

		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadeIcon[0]), origScale.X);
		Engine::SetDvar(GetPatch(xbLive.pPatches->WAW.DrawGrenadeIcon[1]), origScale.Y);
	}
}
		...
		if (vars.bDroppedItems || vars.bEquipment) {
			for (int i = Engine::GetServerInfo()->iMaxClients; i < 0x7FF; i++) {
				...
				if (Engine::IsAlive(i, true)) {
					if (vars.bEquipment) {
						if (*(int*)((DWORD)&entity[i] + 0x160) == ET_MISSLE) {
					...
					if (vars.bDroppedItems) {
						if (*(int*)((DWORD)&entity[i] + 0x160) == ET_ITEM) {
							int index = Engine::FixDroppedItemIndex(*(int*)((DWORD)&entity[i] + 0x208));
