...
namespace MainMenuVars {
	...
	void NoRecoil() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->WAW.NoRecoil), vars.bNoRecoil ? 0x60000000 : 0x4BFA5E4D);
	}

	void NoSway() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->WAW.NoSway), vars.bNoSway ? 0x60000000 : 0x4BFFF77D);
	}

	void NoFlinch() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->WAW.NoFlinch), vars.bNoFlinch ? 0x60000000 : 0x4BFFF845);
	}

	void ConstantRadar() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->WAW.ConstantRadar), vars.bConstantRadar ? 0x38600001 : 0x48169935);
	}

	void Laser() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->WAW.Laser), vars.bLaser ? 0x48000024 : 0x39600000);
	}
	...
	void NoSpread(UserCommand* command) {
		if (*(float*)((DWORD)Engine::GetCG() + 0xECF5C) == 0.f) return;
