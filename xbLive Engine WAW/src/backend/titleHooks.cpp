...
void Menu_PaintAllHook(UIContext* pContext) {
	pGame.pContext = (UIContext*)GetPatch(xbLive.pPatches->WAW.Menu_PaintAllPointer);
...
bool Title::ApplyHooks() {
	if (!bInitialized) {
		*(DWORD*)GetPatch(xbLive.pPatches->WAW.Com_error) = 0x4e800020; // Com_error
		...
		OriginalR_EndFrame.SetupDetour(GetPatch(xbLive.pPatches->WAW.R_EndFrame), R_EndFrameHook);
