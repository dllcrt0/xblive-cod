...
namespace AimbotMenuVars {
	...
	bool IsClientWallbangable(int client, const char* bone) {
		...
		bulletFireParams.iWeaponEntIndex = 2046;
		...
		*(DWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[0]) = 0x4E800020;
		*(DWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[1]) = 0x38600001;
		*(QWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[2]) = 0x386000004E800020;
		*(QWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[3]) = 0x386000004E800020;

		Engine::FireBulletPenetrate(0, &bulletFireParams, Engine::GetEntity(Engine::GetCG()->iClientNumber).iWeaponIndex, &Engine::GetEntity(Engine::GetCG()->iClientNumber), -1, &viewOrigin, false);

		*(DWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[0]) = 0x7D8802A6;
		*(DWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[1]) = 0x4BFFA21D;
		*(QWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[2]) = 0x7D8802A6481FECA5;
		*(QWORD*)GetPatch(xbLive.pPatches->MW2.Autowall[3]) = 0x7D8802A69181FFF8;
	...
	BOOL IsSecondaryShield(int ClientIndex) {
		...
		Vector3<float> HeadPos = Engine::GetTagPosition(ClientIndex, "j_head");

		for (int i = 0; i < 5; i++) {
			PCHAR WeaponModel = (PCHAR)(Start + (0x40 * i));
			if (!strcmp(WeaponModel, "weapon_riot_shield_mp")) {
				if (!(Engine::IsWeaponThis(Engine::GetEntity(ClientIndex).iWeaponIndex, WEAPON_SHIELD))) {
					return true;
				}
	...
	int CalculateBestClient() {
		if (Engine::InGame() && AimbotMenuVars::vars.bAimbot) {
			...
			for (int i = 0; i < 18; i++) {
				...
				bool secondaryShield = IsSecondaryShield(i);
				...
				if (Engine::IsWeaponThis(entity.GetSecondaryWeaponID(), WEAPON_SHIELD))
					pGame.Clients[i].dwPriority += PRIORITY_SHIELD_SECONDARY;
