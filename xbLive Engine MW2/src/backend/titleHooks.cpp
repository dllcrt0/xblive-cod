...
void R_EndFrameHook() {
	pGame.pContext = (UIContext*)GetPatch(xbLive.pPatches->MW2.Menu_PaintAllPointer);
...
bool Title::ApplyHooks() {
	if (!bInitialized) {
		...
		*(DWORD*)GetPatch(xbLive.pPatches->MW2.CrashFix[1]) = 0x60000000; // crash
		...
		OriginalR_EndFrame.SetupDetour(GetPatch(xbLive.pPatches->MW2.R_EndFrame), R_EndFrameHook);
