...
typedef struct _TLS_VAL_DATA {
	PVOID data; // 0x0 sz:0x4
} TLS_VAL_DATA, *PTLS_VAL_DATA;

typedef struct _TLS_INFO {
	TLS_VAL_DATA tls_value[4]; // 0x0 sz:0x16
	DWORD reserved; // 0x10 sz:0x4
	PVOID data; // 0x14 sz:0x4
} TLS_INFO, *PTLS_INFO;

typedef struct _KPCR {
	PTLS_INFO TlsData; // 0x0 sz:0x4
	union {
		struct {
