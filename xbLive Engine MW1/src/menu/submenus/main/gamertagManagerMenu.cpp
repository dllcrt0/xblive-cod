...
namespace GamertagManagerMenuVars {
	...
	void SetGamertag(const char* pName) {
		...
		sprintf(szPayload, "userinfo \"\\rate\\20000\\snaps\\20\\name\\%s\\xuid\\%s\\natType\\1\\0\"", pName, (char*)(GetPatch(xbLive.pPatches->MW1.XenonUserData) + 0x30));
		Engine::Cbuf_AddText(0, szPayload);
		strcpy((char*)(GetPatch(xbLive.pPatches->MW1.LocalNamePointer) + 0x4), pName);
	}

	void SetGamertagColor(const char* pName, const char* Color) {
		char szPayload[0x100];
		sprintf(szPayload, "userinfo \"\\rate\\20000\\snaps\\20\\name\\%s\\clanAbbrev\\%s\\xuid\\%s\\natType\\1\\rank\\1\\0\"", pName, Color, (char*)(GetPatch(xbLive.pPatches->MW1.XenonUserData) + 0x30));
		Engine::Cbuf_AddText(0, szPayload);
		strcpy((char*)(GetPatch(xbLive.pPatches->MW1.LocalNamePointer) + 0x4), pName);
	}
	...
	void SetColor() {
		SetGamertagColor((char*)(GetPatch(xbLive.pPatches->MW1.LocalNamePointer) + 0x4), ScrollNameColors[vars.iColor].Result);
	}
}
...
void GamertagManagerMenu::UpdateOnce() {
	...
	call_once(&init, [] {
		strcpy(vars.szDefault, (char*)(GetPatch(xbLive.pPatches->MW1.LocalNamePointer) + 0x4));
	});

	vars.vSavedGamertags.clear();

	FILE* gamertagFile = fopen(GAME_PATH "Gamertags.txt", "r");
	if (gamertagFile) {
		DWORD dwCount = 0;
...
void GamertagManagerMenu::FeatureUpdate() {
	if (vars.bRainbowGamertag) {
		...
		if (GetTickCount() - timer > vars.iRainbowDelay) {
			...
			SetGamertagColor((char*)(GetPatch(xbLive.pPatches->MW1.LocalNamePointer) + 0x4), ScrollNameColors[current].Result);
