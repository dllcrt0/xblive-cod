...
namespace AimbotMenuVars {
	...
	bool IsClientWallbangable(int client, const char* bone) {
		...
		*(DWORD*)GetPatch(xbLive.pPatches->MW1.Autowall[0]) = 0x4E800020;
		*(DWORD*)GetPatch(xbLive.pPatches->MW1.Autowall[1]) = 0x4E800020;

		Engine::FireBulletPenetrate(0, &bulletFireParams, Engine::GetWeaponDef(Engine::GetEntity(Engine::GetCG()->iClientNumber).iWeaponIndex), &Engine::GetEntity(Engine::GetCG()->iClientNumber), &viewOrigin, false);

		*(DWORD*)GetPatch(xbLive.pPatches->MW1.Autowall[0]) = 0x7D8802A6;
		*(DWORD*)GetPatch(xbLive.pPatches->MW1.Autowall[1]) = 0x7D8802A6;
