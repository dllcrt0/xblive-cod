...
namespace MainMenuVars {
	...
	void NoRecoil() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->MW1.NoRecoil), vars.bNoRecoil ? 0x60000000 : 0x4803DD41);
	}

	void NoSway() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->MW1.NoSway[0]), vars.bNoSway ? 0x60000000 : 0x4BFFB3DD);
		Native::Write4Byte(GetPatch(xbLive.pPatches->MW1.NoSway[1]), vars.bNoSway ? 0x60000000 : 0x4BFFC3B5);
	}

	void NoFlinch() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->MW1.NoFlinch), vars.bNoFlinch ? 0x60000000 : 0x4BFFF8A5);
	}

	void ConstantRadar() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->MW1.ConstantRadar), vars.bConstantRadar ? 0x38600001 : 0x4BEAF075);
	}

	void Laser() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->MW1.Laser), vars.bLaser ? 0x48000024 : 0x39400000);
	}
	...
	void NoSpread(UserCommand* oldCommand, UserCommand* nextCommand) {
		if (*(float*)((DWORD)Engine::GetCG() + 0xF09DC) == 0.f) return;
