...
void Renderer::Initialize() {
	...
	Materials.compass_map_default = Engine::R_RegisterMaterial("compass_map_default", 0);
	Materials.objective_line = Engine::R_RegisterMaterial("objective_line", 0);
	Materials.compassping_player = Engine::R_RegisterMaterial("compassping_player", 0);
	Materials.compass_objpoint_airstrike_busy = Engine::R_RegisterMaterial("compass_objpoint_airstrike_busy", 0);
	Materials.compass_objpoint_helicopter_busy = Engine::R_RegisterMaterial("compass_objpoint_helicopter_busy", 0);
	Materials.compass_objpoint_airstrike_friendly = Engine::R_RegisterMaterial("compass_objpoint_airstrike_friendly", 0);
	Materials.compass_objpoint_helicopter_friendly = Engine::R_RegisterMaterial("compass_objpoint_helicopter_friendly", 0);

	/*Fonts*/
	Fonts.bigDevFont = Engine::R_RegisterFont("fonts/bigDevFont", 0);
	Fonts.smallDevFont = Engine::R_RegisterFont("fonts/smallDevFont", 0);
	Fonts.consoleFont = Engine::R_RegisterFont("fonts/consoleFont", 0);
	Fonts.hudSmallFont = Engine::R_RegisterFont("fonts/hudsmallfont", 0);
	Fonts.hudBigFont = Engine::R_RegisterFont("fonts/hudbigfont", 0);
	Fonts.objectiveFont = Engine::R_RegisterFont("fonts/objectivefont", 0);
	Fonts.extraBigFont = Engine::R_RegisterFont("fonts/extrabigfont", 0);
	Fonts.normalFont = Engine::R_RegisterFont("fonts/normalfont", 0);
	Fonts.boldFont = Engine::R_RegisterFont("fonts/boldfont", 0);
	Fonts.smallFont = Engine::R_RegisterFont("fonts/smallfont", 0);
	Fonts.bigFont = Engine::R_RegisterFont("fonts/bigfont", 0);
...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->MW1.ScreenPlacement), X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}
...
void Renderer::RenderHeader() {
	...
	DrawString("v1.0", 845.f, 149.f, Fonts.normalFont, 0.45f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
...
void Renderer::RenderSubHeader() {
	...
	DrawString("Main", 391.5f + (tabSpace * 1), 170, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Aimbot", 391.5f + (tabSpace * 2), 170, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("HvH", 391.5f + (tabSpace * 3), 170, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Visuals", 391.5f + (tabSpace * 4), 170, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Players", 391.5f + (tabSpace * 5), 170, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Settings", 391.5f + (tabSpace * 6), 170, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
}
...
void Renderer::RenderFooter() {
	...
	if (strTooltip.length() == 0) {
		...
		string tip = WordWrap(strTooltip, Fonts.normalFont, 0.50f, width - 25, &iTooltipLines);
		DrawOutlinedRect(pMenu.vCenter.X - (width / 2), y, width, iTooltipLines == 1 ? fOptionHeight * 1.35f : iTooltipLines * (fOptionHeight), 1, Color(0, 0, 0, 150), Color(160, 57, 56));
		DrawString(tip.c_str(), pMenu.vCenter.X - (width / 2) + 8, y + 22, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyLeft);
	}
}
