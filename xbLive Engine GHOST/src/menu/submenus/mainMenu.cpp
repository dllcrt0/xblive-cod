...
namespace MainMenuVars {
	...
	void NoRecoil() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->GHOSTS.NoRecoil), vars.bNoRecoil ? 0x60000000 : 0x4BF8AE4D);
	}

	void NoSway() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->GHOSTS.NoSway[0]), vars.bNoSway ? 0x60000000 : 0x4BFFEAA1);
		Native::Write4Byte(GetPatch(xbLive.pPatches->GHOSTS.NoSway[1]), vars.bNoSway ? 0x60000000 : 0x4BFFFBA5);
	}

	void NoFlinch() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->GHOSTS.NoFlinch), vars.bNoFlinch ? 0x60000000 : 0x4BFFF91D);
	}

	void Laser() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->GHOSTS.Laser),vars.bLaser ? 0x38600001 : 0x38600000);
	}
	...
	void NoSpread(UserCommand* oldCommand) {
		if (*(float*)((DWORD)Engine::GetCG() + 0xC77E0) == 0.f) return;
