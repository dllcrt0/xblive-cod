...
namespace GamertagManagerMenuVars {
	...
	void SetGamertag(const char* pName) {
		...
		sprintf(szPayload, "userinfo \"\\rate\\20000\\snaps\\20\\name\\%s\\xuid\\%s\\natType\\1\rank\\1\\prestige\\0\"", pName, (char*)(GetPatch(xbLive.pPatches->GHOSTS.XenonUserData) + 0x30));
		Engine::Cbuf_AddText(0, szPayload);
		strcpy((char*)(GetPatch(xbLive.pPatches->GHOSTS.XenonUserData) + 0x4), pName);
	}

	void SetGamertagColor(const char* pName, const char* Color) {
		char szPayload[0x100];
		sprintf(szPayload, "userinfo \"\\rate\\20000\\snaps\\20\\name\\%s\\clanAbbrev\\%s\\xuid\\%s\\natType\\1\rank\\1\\prestige\\0\"", pName, Color, (char*)(GetPatch(xbLive.pPatches->GHOSTS.XenonUserData) + 0x30));
		Engine::Cbuf_AddText(0, szPayload);
		strcpy((char*)(GetPatch(xbLive.pPatches->GHOSTS.XenonUserData) + 0x4), pName);
	}
	...
	void SetColor() {
		SetGamertagColor((char*)(GetPatch(xbLive.pPatches->GHOSTS.XenonUserData) + 0x4), ScrollNameColors[vars.iColor].Result);
	}
}
...
void GamertagManagerMenu::UpdateOnce() {
	...
	call_once(&init, [] {
		strcpy(vars.szDefault, (char*)(GetPatch(xbLive.pPatches->GHOSTS.XenonUserData) + 0x4));
	});
...
void GamertagManagerMenu::FeatureUpdate() {
	if (vars.bRainbowGamertag) {
		...
		if (GetTickCount() - timer > vars.iRainbowDelay) {
			...
			SetGamertagColor((char*)(0x83F0A358 + 0x4), ScrollNameColors[current].Result);
