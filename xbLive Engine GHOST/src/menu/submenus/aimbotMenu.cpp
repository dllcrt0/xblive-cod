...
namespace AimbotMenuVars {
	...
	bool IsClientWallbangable(int client, const char* bone) {
		...
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[0]) = 0x4e800020;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[1]) = 0x4e800020;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[2]) = 0x38600001;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[3]) = 0x60000000;

		Engine::FireBulletPenetrate(0, &bulletFireParams, Engine::GetEntity(Engine::GetCG()->iClientNumber).iWeaponIndex, 0, &Engine::GetEntity(Engine::GetCG()->iClientNumber), -1, &viewOrigin, false);

		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[0]) = 0x4e800020;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[1]) = 0x7D8802A6;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[2]) = 0x4BFFA9AD;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.Autowall[3]) = 0x4BFFE9C9;
