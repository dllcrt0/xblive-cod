...
/*Background update*/
void VisualMenu::FeatureUpdatePaintAll() {
	if (Engine::InGame()) {
		// entity loops
			...
			if (vars.bDrawEquipment) {
				if (Engine::IsAlive(i, true) && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
					if (Engine::IsWeaponEquipmentItem(entity[i].iWeaponIndex)) {
						Vector3<float> topLocation, topLocationOut, mins, maxs;
						Engine::CG_GetEntityBModelBounds(&entity[i], &mins, &maxs);
						topLocation = entity[i].vOrigin;
						topLocation.Z -= mins.Z;

						if (Engine::WorldToScreen(topLocation, &topLocationOut)) {
							Vector2<float> aspect;
							auto icon = Engine::GetWeaponMaterialWithAspect(entity[i].iWeaponIndex, &aspect);
							if (icon) {
								Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->GHOSTS.ScreenPlacement), topLocationOut.X - 20, topLocationOut.Y - 20, 20, 20, 0.0, vars.DrawEquipmentColor.GetFloatPtr(), icon);
							}
						}
					}
				}
				...
				if (Engine::IsAlive(i, true) && entity[i].Type == ET_ITEM && (Engine::GetDistance(Engine::GetCG()->Refdef.vOrigin, entity[i].vOrigin) < 1500.0f)) {
					...
					if (Engine::WorldToScreen(topLocation, &topLocationOut)) {
						...
						if (!scav) {
							...
							auto icon = Engine::GetWeaponMaterialWithAspect(entity[i].iWeaponIndex, &aspect);
							if (icon) {
								Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->GHOSTS.ScreenPlacement), topLocationOut.X - (aspect.X / 2), topLocationOut.Y - (aspect.Y / 2), aspect.X, aspect.Y, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), icon);
							}
						} else {
							Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->GHOSTS.ScreenPlacement), topLocationOut.X - 45, topLocationOut.Y - 10, 45, 30, 0.0, vars.DrawDroppedItemsColor.GetFloatPtr(), pRenderer.Materials.scavenger_pickup);
						}
					}
				}
			}
		...
		if (vars.bDrawHealthBar) {
			if (Engine::GetCG()->iMaxHealth != 0) {
				...
				pRenderer.DrawString(pUtils.va("%i / %i", Engine::GetCG()->iHealth, Engine::GetCG()->iMaxHealth), 640, 701, pRenderer.Fonts.normalFont, 0.53f, Color(0xFFFFFFFF), false, false, JustifyCenter);
			}
		}
	}
}
