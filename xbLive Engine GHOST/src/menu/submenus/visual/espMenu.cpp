...
namespace ESPMenuVars {
	...
	void PatchGameNames() {
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.PatchGameNames[0]) = (vars.bDrawNames || vars.bDrawDistance) ? 0x60000000 : 0x4BFFE9B1;
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.PatchGameNames[1]) = (vars.bDrawNames || vars.bDrawDistance) ? 0x60000000 : 0x4BFFE7BD;
	}
	...
	if (Engine::InGame()) {
				...
				if ((vars.bDrawFriendlies && Engine::IsFriend(i)) || (vars.bDrawEnemies && !Engine::IsFriend(i))) {
					...
					if (vars.bDrawNames) {
						...
						if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
							...
							FixTextForDistance(Engine::GetCG()->ClientName[i].szName, &headPosOut.X, &headPosOut.Y, pRenderer.Fonts.smallFont, 1, feetPos, &distanceOut);
							auto x = pRenderer.CalculateXValueWithAlign(feetPosBackup.X, Engine::GetCG()->ClientName[i].szName, pRenderer.Fonts.smallFont, 0.5f, JustifyCenter);

							if (vars.bDrawDistance) {
								pRenderer.DrawRect(x - 3, headPosOut.Y - 27 - 8, (Engine::R_TextWidth(Engine::GetCG()->ClientName[i].szName, pRenderer.Fonts.smallFont) * 0.5f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
								pRenderer.DrawString(Engine::GetCG()->ClientName[i].szName, x - 1, headPosOut.Y - 16 - 3, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);
							} else {
								pRenderer.DrawRect(x - 3, headPosOut.Y - 16 - 3, (Engine::R_TextWidth(Engine::GetCG()->ClientName[i].szName, pRenderer.Fonts.smallFont) * 0.5f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
								pRenderer.DrawString(Engine::GetCG()->ClientName[i].szName, x - 1, headPosOut.Y - 5 + 1 + 1, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);
							}
						}
					}
					...
					if (vars.bDrawDistance) {
						...
						if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
							...
							pRenderer.DrawRect(x - 3, headPosOut.Y - 16 - 3, (Engine::R_TextWidth(buffer, pRenderer.Fonts.smallFont) * 0.5f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
							pRenderer.DrawString(buffer, x - 1, headPosOut.Y - 5 + 1 + 1, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);
						}
					}
				}
			}
		...
		if (vars.bDrawDogs) {
			for (int i = Engine::GetServerInfo()->iMaxClients; i < 0x7FF; i++) {
				auto entity = Engine::GetEntityPtr();
				if (Engine::IsAlive(i)) {
					if (entity[i].Type == 18 && !Engine::GetTagPosition(i, "l_tail0_jnt").IsNull()) {
						bool isFriend = Engine::GetCG()->ClientTeam[Engine::GetCG()->iClientNumber].iTeam == entity->GetDogTeam();
						if ((vars.bDrawFriendlies && isFriend) || (vars.bDrawEnemies && !isFriend)) {
							...
							ESPMenuVars::DrawBoneOnPlayer(i, "r_frontleg0_jnt", "r_frontleg1_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "r_frontleg1_jnt", "r_frontleg2_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "r_frontleg2_jnt", "r_frontleg3_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "r_frontleg3_jnt", "r_frontleg4_jnt", color);
							//
							ESPMenuVars::DrawBoneOnPlayer(i, "l_frontleg0_jnt", "l_frontleg1_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_frontleg1_jnt", "l_frontleg2_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_frontleg2_jnt", "l_frontleg3_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_frontleg3_jnt", "l_frontleg4_jnt", color);

							ESPMenuVars::DrawBoneOnPlayer(i, "r_backleg0_jnt", "r_backleg1_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "r_backleg1_jnt", "r_backleg2_jnt", color);
							//
							ESPMenuVars::DrawBoneOnPlayer(i, "l_backleg0_jnt", "l_backleg1_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_backleg1_jnt", "l_backleg2_jnt", color);

							ESPMenuVars::DrawBoneOnPlayer(i, "spine2_jnt", "spine3_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "spine3_jnt", "spine4_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_tail0_jnt", "spine2_jnt", color);

							ESPMenuVars::DrawBoneOnPlayer(i, "l_foot0_jnt", "l_footend_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "r_foot0_jnt", "r_footend_jnt", color);

							ESPMenuVars::DrawBoneOnPlayer(i, "l_tail0_jnt", "l_tail1_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_tail1_jnt", "l_tail2_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_tail2_jnt", "l_tail3_jnt", color);
							ESPMenuVars::DrawBoneOnPlayer(i, "l_tail3_jnt", "l_tailend_jnt", color);
							...
							if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
								...
								pRenderer.DrawRect(x - 3, headPosOut.Y - 27 - 8, (Engine::R_TextWidth("Dog", pRenderer.Fonts.smallFont) * 0.5f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
								pRenderer.DrawString("Dog", x - 1, headPosOut.Y - 16 - 3, pRenderer.Fonts.smallFont, 0.5f, color, false, false, JustifyLeft);
