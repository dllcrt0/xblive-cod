	...
	if (Engine::InGame()) {
		...
		if (GetTickCount() - timer > 250) {
			...
			for (int i = 0; i < Engine::GetServerInfo()->iMaxClients; i++) {
				if (Engine::GetCG()->ClientName[i].szName) {
					vars.players.push_back(Player(i, Engine::GetCG()->ClientName[i].szName, Engine::GetDistance(Engine::GetEntity(i).vOrigin, Engine::GetEntity(Engine::GetCG()->iClientNumber).vOrigin) * 0.03048f));
				}
			}
