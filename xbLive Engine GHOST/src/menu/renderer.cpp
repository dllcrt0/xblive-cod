...
void Renderer::DrawTopRight(const char* text, int pos) {
	...
	DrawString(text, CalculateXValueWithAlign(1265.f, text, Fonts.normalFont, 0.54f, JustifyLeft), 21.f + (pos * 29.f), Fonts.normalFont, 0.54f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->GHOSTS.ScreenPlacement), X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}
...
void Renderer::RenderSubHeader() {
	...
	DrawString("Main", 391.5f + (tabSpace * 1), 171, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Aimbot", 391.5f + (tabSpace * 2), 171, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("HvH", 391.5f + (tabSpace * 3), 171, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Visuals", 391.5f + (tabSpace * 4), 171, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Players", 391.5f + (tabSpace * 5), 171, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Settings", 391.5f + (tabSpace * 6), 171, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyCenter);
}
