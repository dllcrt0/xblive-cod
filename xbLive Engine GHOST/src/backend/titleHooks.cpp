...
typedef int(*tMenu_PaintAll)(int screenDisp, int r4, int r5);
...
int Menu_PaintAllHook(int screenDisp, int r4, int r5) {
	...
	CompassMenu::GetInstance()->FeatureUpdatePaintAll();
	...
	return OriginalMenu_PaintAll(screenDisp, r4, r5);
}

typedef void(*tR_EndFrame)();
tR_EndFrame OriginalR_EndFrame;
void R_EndFrameHook() {
	pGame.pContext = (UIContext*)GetPatch(xbLive.pPatches->GHOSTS.Menu_PaintAllPointer);
		...
		if (AimbotMenuVars::vars.bAimbot) {
			...
			if (pGame.iAimbotTarget != -1) {
				...
				if (AimbotMenuVars::vars.bPositionPrediction) {
					auto oldPosition = entity.vLastOrigin;
					auto newPosition = entity.vNewOrigin;
...
void SetupMinimapHook(DWORD r3, DWORD r4) {
	CompassMenuVars::vars.CurrentlyRenderedType = (CompassType)*(BYTE*)(r4 + 0x58);

	if (CompassMenuVars::vars.bDrawCustomCompass) {
		Material mat;
		RectDef rect;
		float color[4];
		Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->GHOSTS.CompassGetVars), r3, r4, &rect, color, &mat, 0);
...
bool Title::ApplyHooks() {
	if (!bInitialized) {
		*(DWORD*)GetPatch(xbLive.pPatches->GHOSTS.DirtyDiskError) = 0x4e800020;

		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.SetupCompass), &SetupCompassHook, (void**)&OriginalSetupCompass);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.SetupMinimapIcons), &SetupMinimapIconsHook, (void**)&OriginalSetupMinimapIcons);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.SetupMinimap), &SetupMinimapHook, (void**)&OriginalSetupMinimap);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.Menu_PaintAll), &Menu_PaintAllHook, (void**)&OriginalMenu_PaintAll);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.CG_EntityEvent), &CG_EntityEventHook, (void**)&OriginalCG_EntityEvent);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.CG_Respawn), &CG_RespawnHook, (void**)&OriginalCG_Respawn);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.CL_CreateNewCommands), &CL_CreateNewCommandsHook, (void**)&OriginalCL_CreateNewCommands);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.CL_SendCmd), &CL_SendCmdHook, (void**)&OriginalCL_SendCmd);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.XamInputGetState), &XamInputGetStateHook, (void**)&OriginalXamInputGetState);
		pHooking.HookFunction(GetPatch(xbLive.pPatches->GHOSTS.R_EndFrame), &R_EndFrameHook, (void**)&OriginalR_EndFrame);
