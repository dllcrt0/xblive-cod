...
public:
	static HANDLE ResolveFunction(const char* pModuleName, DWORD dwOrdinal) {
		return pInvoker.Invoke<HANDLE>(0x10000000, pModuleName, dwOrdinal);
	}
	...
	static BYTE* GetCheatPatches(DWORD dwTitle, DWORD dwTimestamp) {
		return pInvoker.Invoke<BYTE*>(0x99999999, dwTitle, dwTimestamp);
	}

	static bool HookFunction(DWORD address, void* hookFunction, void** trampoline, BYTE* hookSection, int* hookCount) {
		return pInvoker.Invoke<bool>(0x99999998, address, hookFunction, trampoline, hookSection, hookCount);
	}
};

extern Native pNative;