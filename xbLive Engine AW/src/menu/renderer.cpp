...
void Renderer::Initialize() {
	...
	Materials.scavenger_pickup = Engine::R_RegisterMaterial("scavenger_pickup", 0);

	/*Fonts*/
	Fonts.bigDevFont = Engine::R_RegisterFont("fonts/bigDevFont", 1);
	Fonts.smallDevFont = Engine::R_RegisterFont("fonts/smallDevFont", 1);
	Fonts.consoleFont = Engine::R_RegisterFont("fonts/consoleFont", 1);
	Fonts.bodyFont = Engine::R_RegisterFont("fonts/bodyFont", 1);

	fOptionHeight = 21.f;
}

void Renderer::DrawTopRight(const char* text, int pos) {
	int textWidth = Engine::R_TextWidth(text, Fonts.bodyFont);
	DrawOutlinedRect(1260.f - (textWidth * 0.80f), (pos * 29.f), (textWidth * 0.80f) + 10.f, 22.f, 3, Color(0, 0, 0, 100), Color(160, 57, 64, 150), true);
	DrawString(text, CalculateXValueWithAlign(1265.f, text, Fonts.bodyFont, 0.80f, JustifyLeft), 21.f + (pos * 29.f), Fonts.bodyFont, 0.80f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->AW.ScreenPlacement[0]), X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}

void Renderer::DrawTitle(const char* pTitle) {
	DrawString(pTitle, pMenu.vCenter.X, 149.f, Fonts.bodyFont, 0.70f, Color(0xFFFFFFFF), false, false, JustifyCenter);
}
...
void Renderer::RenderHeader() {
	...
	DrawString("xbLive.online", 433.f, 149.f, Fonts.bodyFont, 0.70f, Color(0xFFFFFFFF), false, false, JustifyLeft);
	DrawString("v1.0", 845.f, 149.f, Fonts.bodyFont, 0.70f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
...
void Renderer::RenderSubHeader() {
	...
	DrawString("Main", 391.5f + (tabSpace * 1), 171, Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Aimbot", 391.5f + (tabSpace * 2), 171, Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("HvH", 391.5f + (tabSpace * 3), 171, Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Visuals", 391.5f + (tabSpace * 4), 171, Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Players", 391.5f + (tabSpace * 5), 171, Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyCenter);
	DrawString("Settings", 391.5f + (tabSpace * 6), 171, Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyCenter);
}
...
void Renderer::RenderFooter() {
	...
	if (strTooltip.length() == 0) {
		...
		string tip = WordWrap(strTooltip, Fonts.bodyFont, 0.76f, width - 25, &iTooltipLines);
		DrawOutlinedRect(pMenu.vCenter.X - (width / 2), y, width, iTooltipLines == 1 ? fOptionHeight * 1.4f : iTooltipLines * (fOptionHeight + 2.f), 1, Color(0, 0, 0, 150), Color(160, 57, 56));
		DrawString(tip.c_str(), pMenu.vCenter.X - (width / 2) + 8, y + 22, Fonts.bodyFont, 0.73f, Color(0xFFFFFFFF), false, false, JustifyLeft);
	}
}
