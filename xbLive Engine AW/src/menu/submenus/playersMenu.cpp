...
namespace PlayersMenuVars {
	...
	const char* GenerateName(Player player) {
		...
		if (player.iID == Engine::GetCG()->bClientNumber) strcat(name, "^1[ME]^7");
	...
	if (Engine::InGame()) {
		...
		if (GetTickCount() - timer > 250) {
			...
			for (int i = 0; i < Engine::GetServerInfo()->iMaxClients; i++) {
				if (Engine::GetCG()->ClientInfo[i].iValid &&  Engine::GetCG()->ClientName2[i].szPlayerName) {
					vars.players.push_back(Player(i, Engine::GetCG()->ClientName2[i].szPlayerName, Engine::GetDistance(Engine::GetEntity(i).vOrigin, Engine::GetEntity(Engine::GetCG()->bClientNumber).vOrigin) * 0.03048f));
				}
			}
