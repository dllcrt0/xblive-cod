...
namespace MainMenuVars {
	...
	void NoRecoil() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->AW.NoRecoil[0]), vars.bNoRecoil ? 0x60000000 : 0x41820020);
		Native::Write4Byte(GetPatch(xbLive.pPatches->AW.NoRecoil[1]), vars.bNoRecoil ? 0x60000000 : 0x895B1869);
	}

	void NoSway() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->AW.NoSway[0]), vars.bNoSway ? 0x60000000 : 0x4BFFFC59);
		Native::Write4Byte(GetPatch(xbLive.pPatches->AW.NoSway[1]), vars.bNoSway ? 0x60000000 : 0x4BFFFD39);
	}

	void NoFlinch() {
		Native::Write4Byte(GetPatch(xbLive.pPatches->AW.NoFlinch), vars.bNoFlinch ? 0x60000000 : 0x4BFFF365);
	}
	...
	void NoSpread(UserCommand* oldCommand) {
		if (*(float*)((DWORD)Engine::GetCG() + 0xBDB80) == 0.f) return;
