...
namespace ESPMenuVars {
	...
	void DisableGameNames() {
		*(DWORD*)GetPatch(xbLive.pPatches->AW.DisableGameNames) = (vars.bDrawNames || vars.bDrawDistance) ? 0x4e800020 : 0x7d8802a6; 
	}
...
void ESPMenu::Init() {
	...
	addOption(ToggleOption("Draw Name")
		...
		.addFunction(DisableGameNames).canBeSaved("ESP")
				...
				if ((vars.bDrawFriendlies && Engine::IsFriend(i)) || (vars.bDrawEnemies && !Engine::IsFriend(i))) {
					...
					if (vars.bDrawBox) {
						...
						if (vars.iDrawBoxSelection == 3) {
							...
							if (Engine::WorldToScreen(position.vOrigin, &origin) && Engine::WorldToScreen(position.vHead, &head)) {
								...
								float width = entity[i].iFlags & 8 ? height / 0.9f : height / 1.95f;
					...
					if (vars.bDrawNames) {
						...
						if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
							...
							auto clientName = Engine::GetCG()->ClientName2[i].szPlayerName;

							FixTextForDistance(clientName, &headPosOut.X, &headPosOut.Y, pRenderer.Fonts.bodyFont, 0.7f, feetPos, &distanceOut);
							auto x = pRenderer.CalculateXValueWithAlign(feetPosBackup.X, clientName, pRenderer.Fonts.bodyFont, 0.7f, JustifyCenter);

							if (vars.bDrawDistance) {
								pRenderer.DrawRect(x - 3, headPosOut.Y - 27 - 8, (Engine::R_TextWidth(clientName, pRenderer.Fonts.bodyFont) * 0.7f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
								pRenderer.DrawString(clientName, x - 2 + 0.5, headPosOut.Y - 16 - 3, pRenderer.Fonts.bodyFont, 0.7f, color, false, false, JustifyLeft);
							} else {
								pRenderer.DrawRect(x - 3, headPosOut.Y - 16 - 3, (Engine::R_TextWidth(clientName, pRenderer.Fonts.bodyFont) * 0.7f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
								pRenderer.DrawString(clientName, x - 2 + 0.5, headPosOut.Y - 5 + 1 + 1, pRenderer.Fonts.bodyFont, 0.7f, color, false, false, JustifyLeft);
							}
						}
					}
					...
					if (vars.bDrawDistance) {
						...
						if (Engine::WorldToScreen(headPos, &headPosOut) && Engine::WorldToScreen(feetPos, &feetPosOut)) {
							...
							char buffer[10];
							_snprintf(buffer, 10, "%.2fm", Engine::GetDistance(entity[i].vOrigin, Engine::GetEntity(Engine::GetCG()->bClientNumber).vOrigin) * 0.03048f);

							FixTextForDistance(buffer, &headPosOut.X, &headPosOut.Y, pRenderer.Fonts.bodyFont, 0.7f, feetPos, &distanceOut);
							auto x = pRenderer.CalculateXValueWithAlign(feetPosBackup.X, buffer, pRenderer.Fonts.bodyFont, 0.7f, JustifyCenter);

							pRenderer.DrawRect(x - 3, headPosOut.Y - 16 - 3, (Engine::R_TextWidth(buffer, pRenderer.Fonts.bodyFont) * 0.7f) + 3, 10 + 4, JustifyLeft, Color(0, 0, 0, 150));
							pRenderer.DrawString(buffer, x - 2 + 0.5, headPosOut.Y - 5 + 1 + 1, pRenderer.Fonts.bodyFont, 0.7f, color, false, false, JustifyLeft);
						}
					}
				}
			}
