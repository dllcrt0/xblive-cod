...
namespace CrosshairMenuVars {
	...
	void DisableEngineCrosshair() {
		*(DWORD*)GetPatch(xbLive.pPatches->AW.DisableEngineCrosshair) = vars.bDisableEngineCrosshair ? 0x4e800020 : 0x7D8802A6; 
	}
}
	...
	if (Engine::InGame()) {
		if (vars.bDrawCustomCrosshair) {
			...
			if (vars.bRotateCustomCrosshair) {
				...
				Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->AW.ScreenPlacement[0]), center.X - (vars.iSize / 2), center.Y, vars.iSize, 2, rotation, vars.CrosshairColor.GetFloatPtr(), pRenderer.Materials.white);
				Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->AW.ScreenPlacement[0]), center.X - (vars.iSize / 2), center.Y, vars.iSize, 2, rotation + 90, vars.CrosshairColor.GetFloatPtr(), pRenderer.Materials.white);
			} else {
				Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->AW.ScreenPlacement[0]), center.X - (vars.iSize / 2), center.Y, vars.iSize, 2, vars.iRotation, vars.CrosshairColor.GetFloatPtr(), pRenderer.Materials.white);
				Engine::CG_DrawRotatedPicPhysical(GetPatch(xbLive.pPatches->AW.ScreenPlacement[0]), center.X - (vars.iSize / 2), center.Y, vars.iSize, 2, vars.iRotation + 90, vars.CrosshairColor.GetFloatPtr(), pRenderer.Materials.white);
			}
		}
	}
}
