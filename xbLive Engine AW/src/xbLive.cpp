...
__declspec(noinline) void GetCheatPatches() {
	...
	if (XexExecutableModuleHandle) {
		...
		if (title) {
			...
			if (data) {
				memcpy(&xbLive.pPatches->AW, data + 4, *(DWORD*)((DWORD)data));
...
void ConstantLoop() {
	...
	GetCheatPatches();
	FunctionObfuscation(GetCheatPatches).Xor(69);
#endif

	if (xbLive.pPatches->bHasPatches) {
		while (!xbLive.bTerminateProcess) {
			if (XamGetCurrentTitleId() == GAME_TITLE && !xbLive.bShutdownCheats) {
